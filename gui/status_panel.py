...
class StatusPanel:
    ...
    def _create_widgets(self):
        ...
        ttk.Label(storage_frame, text="Time Until Next Block:").grid(row=3, column=0, sticky=tk.W, padx=5, pady=5)
        self.next_block_label = ttk.Label(storage_frame, text="00:00:00")
        self.next_block_label.grid(row=3, column=1, sticky=tk.W, padx=5, pady=5)
        
        # Estimated block size
        ttk.Label(storage_frame, text="Estimated Block Size:").grid(row=4, column=0, sticky=tk.W, padx=5, pady=5)
        self.block_estimate_label = ttk.Label(storage_frame, text="0 MB")
        self.block_estimate_label.grid(row=4, column=1, sticky=tk.W, padx=5, pady=5)
        
        # Current block size
        ttk.Label(storage_frame, text="Current Block Size:").grid(row=5, column=0, sticky=tk.W, padx=5, pady=5)
        self.block_size_label = ttk.Label(storage_frame, text="0 bytes")
        self.block_size_label.grid(row=5, column=1, sticky=tk.W, padx=5, pady=5)
        
        # Estimated daily storage
        ttk.Label(storage_frame, text="Daily Storage Estimate:").grid(row=6, column=0, sticky=tk.W, padx=5, pady=5)
        self.day_size_label = ttk.Label(storage_frame, text="0 MB")
        self.day_size_label.grid(row=6, column=1, sticky=tk.W, padx=5, pady=5)
        
        # Estimated 90-day storage
        ttk.Label(storage_frame, text="90-Day Storage Estimate:").grid(row=7, column=0, sticky=tk.W, padx=5, pady=5)
        self.storage_estimate_label = ttk.Label(storage_frame, text="0 GB")
        self.storage_estimate_label.grid(row=7, column=1, sticky=tk.W, padx=5, pady=5)
        
        # Recordings folder size
        ttk.Label(storage_frame, text="Recordings Folder Size:").grid(row=8, column=0, sticky=tk.W, padx=5, pady=5)
        self.folder_size_label = ttk.Label(storage_frame, text="0 MB")
        self.folder_size_label.grid(row=8, column=1, sticky=tk.W, padx=5, pady=5)
        
        # Free disk space
        ttk.Label(storage_frame, text="Free Disk Space:").grid(row=9, column=0, sticky=tk.W, padx=5, pady=5)
        self.free_space_label = ttk.Label(storage_frame, text="0 GB")
        self.free_space_label.grid(row=9, column=1, sticky=tk.W, padx=5, pady=5)
        
        # Retention fit
        ttk.Label(storage_frame, text="Retention Would Fit:").grid(row=10, column=0, sticky=tk.W, padx=5, pady=5)
        self.retention_fit_label = ttk.Label(storage_frame, text="Calculating...")
        self.retention_fit_label.grid(row=10, column=1, sticky=tk.W, padx=5, pady=5)
    ...
    def update_status(self, status):
        ...
        if "time_until_next_block" in status and status["recording"] and not status["paused"]:
            ...
            minutes, seconds = divmod(remainder, 60)
            self.next_block_label.config(text=f"{hours:02d}:{minutes:02d}:{seconds:02d}")
        else:
            self.next_block_label.config(text="00:00:00")
